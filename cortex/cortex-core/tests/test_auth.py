...
def test_verify_endpoint():
    ...
    # First, get a token
    login_response = client.post("/auth/login", data={"username": "user@example.com", "password": "password123"})
...
def test_protected_endpoint():
    ...
    response = client.get("/config/user/profile", headers={"Authorization": f"Bearer {token}"})
    assert response.status_code == 200
    ...
    # Test with invalid token
    response = client.get("/config/user/profile", headers={"Authorization": "Bearer invalid-token"})
    assert response.status_code == 401
