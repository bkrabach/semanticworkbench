...
from app.models.domain.pydantic_ai import Conversation
...
def test_model_class(conversation_repo):
    ...
    assert conversation_repo.model_class == ConversationModel


def test_domain_model_class(conversation_repo):
    """Test that the repository uses the correct domain model class."""
    assert conversation_repo.domain_model_class == Conversation


def create_conversation_model(conversation_id="test-conv-id", workspace_id="test-ws-id", user_id="test-user-id"):
    """Create a conversation model for testing."""
    return ConversationModel(
        id=conversation_id, workspace_id=workspace_id, user_id=user_id, topic="Test Conversation", metadata={}
    )


def create_conversation_domain(conversation_id="test-conv-id", workspace_id="test-ws-id", user_id="test-user-id"):
...
async def test_create_conversation(conversation_repo, mock_session):
    ...
    assert result.user_id == conversation.user_id
...
async def test_get_by_id(conversation_repo, mock_session):
    ...
    # Create a mock result with a scalar method
    mock_result = MagicMock()
    mock_result.scalar_one_or_none = MagicMock(return_value=conversation_model)
    ...
    assert result.user_id == conversation_model.user_id
...
async def test_delete_conversation(conversation_repo, mock_session):
    ...
    # Call delete method
    result = await conversation_repo.delete(create_conversation_domain())
...
async def test_delete_not_found(conversation_repo, mock_session):
    ...
    with pytest.raises(ResourceNotFoundException) as exc_info:
        await conversation_repo.delete(create_conversation_domain())
...
async def test_list_by_workspace(conversation_repo, mock_session):
    ...
    # Setup mock session behavior
    conversation_models = [
        create_conversation_model("conv1", "ws1", "user1"),
        create_conversation_model("conv2", "ws1", "user1"),
    ]

    # Create a mock result with a scalars method
    mock_result = MagicMock()
    mock_result.scalars = MagicMock(return_value=conversation_models)

    # Set up the execute to return our mock result
    mock_session.execute = AsyncMock(return_value=mock_result)

    # Call list_by_workspace method
    result = await conversation_repo.list_by_workspace("ws1", "user1")
