...
# Define a protocol that represents the required methods of the memory service
class MemoryServiceProtocol(Protocol):
    async def get_history(self, user_id: str) -> List[Dict[str, Any]]: ...
    async def get_limited_history(self, user_id: str, **kwargs: Any) -> List[Dict[str, Any]]: ...
    async def get_conversation(self, conversation_id: str) -> List[Dict[str, Any]]: ...


@pytest.fixture
def mock_memory_service() -> MemoryServiceProtocol:
    """Create a mock Memory Service."""
    # Create a mock following the protocol
    ...
    # Cast to help mypy understand this is a valid MemoryServiceProtocol
    return cast(MemoryServiceProtocol, memory_service)


# Create a derived class for testing to avoid mypy errors with Optional[Any]
class TestCognitionService(CognitionService):
    """Subclass for testing that makes memory_service a required non-None attribute."""

    def __init__(self, memory_service: MemoryServiceProtocol) -> None:
        """Initialize with a required memory service."""
        super().__init__(memory_service=memory_service)
        self.memory_service = memory_service  # Override Optional[Any] with a known non-None value


@pytest.fixture
def cognition_service(mock_memory_service: MemoryServiceProtocol) -> TestCognitionService:
    """Create a Cognition Service with mocked dependencies."""
    service = TestCognitionService(memory_service=mock_memory_service)
    return service
...
@pytest.mark.asyncio
async def test_initialization(cognition_service: CognitionService) -> None:
...
@pytest.mark.asyncio
async def test_initialization_no_memory_service() -> None:
...
@pytest.mark.asyncio
async def test_shutdown(cognition_service: CognitionService) -> None:
...
@pytest.mark.asyncio
async def test_get_context_empty_user_id(cognition_service: CognitionService) -> None:
...
@pytest.mark.asyncio
async def test_get_context_no_history(cognition_service: CognitionService) -> None:
...
@pytest.mark.asyncio
async def test_get_context_with_history(cognition_service: CognitionService) -> None:
    ...
    history = [
        create_mock_message(
            message_id="msg1", content="Hello world", timestamp=(now - timedelta(minutes=10)).isoformat()
        ),
        create_mock_message(
            message_id="msg2", content="How are you today?", timestamp=(now - timedelta(minutes=5)).isoformat()
        ),
...
@pytest.mark.asyncio
async def test_get_context_with_query(cognition_service: CognitionService) -> None:
...
@pytest.mark.asyncio
async def test_get_context_recency_weight(cognition_service: CognitionService) -> None:
    ...
    history = [
        create_mock_message(
            message_id="msg1", content="Python tutorial", timestamp=(now - timedelta(minutes=10)).isoformat()
        ),
        create_mock_message(
            message_id="msg2", content="How are you today?", timestamp=(now - timedelta(minutes=5)).isoformat()
        ),
...
@pytest.mark.asyncio
async def test_analyze_conversation_invalid_params(cognition_service: CognitionService) -> None:
...
@pytest.mark.asyncio
async def test_analyze_conversation_not_found(cognition_service: CognitionService) -> None:
...
@pytest.mark.asyncio
async def test_analyze_conversation_summary(cognition_service: CognitionService) -> None:
    ...
    conversation = [
        ...
        create_mock_message(
            user_id="user1", content="How are you?", timestamp=(now - timedelta(minutes=8)).isoformat()
        ),
    ...
    result = await cognition_service.analyze_conversation(
        user_id="user1", conversation_id="conv1", analysis_type="summary"
    )
...
@pytest.mark.asyncio
async def test_analyze_conversation_topics(cognition_service: CognitionService) -> None:
    ...
    result = await cognition_service.analyze_conversation(
        user_id="user1", conversation_id="conv1", analysis_type="topics"
    )
...
@pytest.mark.asyncio
async def test_analyze_conversation_sentiment(cognition_service: CognitionService) -> None:
    ...
    positive_result = await cognition_service.analyze_conversation(
        user_id="user1", conversation_id="conv1", analysis_type="sentiment"
    )
    ...
    negative_result = await cognition_service.analyze_conversation(
        user_id="user1", conversation_id="conv1", analysis_type="sentiment"
    )
...
@pytest.mark.asyncio
async def test_analyze_conversation_unknown_type(cognition_service: CognitionService) -> None:
    ...
    result = await cognition_service.analyze_conversation(
        user_id="user1", conversation_id="conv1", analysis_type="unknown_type"
    )
...
@pytest.mark.asyncio
async def test_search_history_invalid_params(cognition_service: CognitionService) -> None:
...
@pytest.mark.asyncio
async def test_search_history_no_results(cognition_service: CognitionService) -> None:
...
@pytest.mark.asyncio
async def test_search_history_with_results(cognition_service: CognitionService) -> None:
...
@pytest.mark.asyncio
async def test_search_history_with_conversation_data(cognition_service: CognitionService) -> None:
    ...
    async def mock_get_conversation(conv_id: str) -> List[Dict[str, Any]]:
        if conv_id == "conv1":
            return conv1_data
        elif conv_id == "conv2":
            return conv2_data
    ...
    # Set the side_effect and ensure proper typing
    get_conversation_mock = AsyncMock(side_effect=mock_get_conversation)
    cognition_service.memory_service.get_conversation = get_conversation_mock
...
@pytest.mark.asyncio
async def test_search_history_without_conversation_data(cognition_service: CognitionService) -> None:
