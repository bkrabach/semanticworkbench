...
from app.utils.exceptions import ResourceNotFoundException, ValidationErrorException, PermissionDeniedException
...
async def create_workspace(
    workspace_data: api_models.WorkspaceCreateRequest,
...
async def create_conversation(
    conversation_data: api_models.ConversationCreateRequest,
    current_user: Dict[str, Any] = Depends(get_current_user)
):
    ...
    if workspace.get("owner_id") != current_user["id"]:
        raise PermissionDeniedException(
            ...
            message="You don't have permission to create conversations in this workspace"
        )
