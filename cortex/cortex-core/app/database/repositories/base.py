...
class BaseRepository(Generic[T, DB]):
    ...
    async def get_by_id(self, entity_id: str) -> Optional[T]:
        ...
        try:
            ...
            query = select(self.db_model_type).where(getattr(self.db_model_type, id_attr_name) == entity_id)
    ...
    async def list(self, filters: Optional[Dict[str, Any]] = None, limit: int = 100, offset: int = 0) -> List[T]:
        ...
        try:
            ...
            domain_entities = [
                entity for entity in [self._to_domain(db_entity) for db_entity in db_entities] if entity is not None
            ]
    ...
    async def update(self, entity: T) -> T:
        ...
        try:
            ...
            # Get existing entity
            query = select(self.db_model_type).where(getattr(self.db_model_type, db_id_attr_name) == entity_id)
            result = await self.session.execute(query)
            db_entity = result.scalars().first()

            if not db_entity:
                raise EntityNotFoundError(f"{self.model_type.__name__} not found with ID: {entity_id}")
