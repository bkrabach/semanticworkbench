...
class FusionMCPAddIn:
    ...
    def start(self):
        ...
        try:
            ...
            self.server_thread = threading.Thread(target=self._run_server, daemon=True, name="MCPServerThread")
    ...
    def stop(self):
        ...
        try:
            ...
            if self.server:
                ...
                # Give time for the server to release resources
                time.sleep(0.5)
            ...
            if self.server_thread and self.server_thread.is_alive():
                ...
                # If thread hasn't exited, log warning but continue
    ...
    def _verify_port_released(self):
        """Verify that the port is released after server shutdown"""
        try:
            import socket
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                ...
                log(f"Port {self.port} successfully released")
        except OSError:
            log(f"WARNING: Port {self.port} is still in use after shutdown")
        except Exception as e:
            self.logger.error(f"Error checking port status: {str(e)}")
